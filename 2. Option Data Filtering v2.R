...
# Load broader color palette
colors <- terrain.colors(100)  # or use rainbow(100), heat.colors(100), etc.
library(lattice)
library(reshape2)
...
# Choose a perceptually uniform and broad palette

# Normalize IV to [0, 1] for broader color mapping
IV_range <- range(df$IV, na.rm = TRUE)
df$IV_scaled <- (df$IV - IV_range[1]) / diff(IV_range)

# Create a broad custom color gradient
colors <- colorRampPalette(c("blue", "cyan", "yellow", "orange", "red", "darkred"))(100)

# Map colors to scaled IV values
df$col <- colors[cut(df$IV_scaled, breaks = 100, labels = FALSE)]

# Plot with dynamic color mapping
wireframe(
  ...
  xlab         = list("Time to Maturity", cex = 1.5, rot = -55),
